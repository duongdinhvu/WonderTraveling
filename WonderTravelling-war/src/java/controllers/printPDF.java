...
public class printPDF extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String transportation = (String) session.getAttribute("Transportation");
        transportation = transportation.toUpperCase();
        short rooms = (short) session.getAttribute("room");
        double unitPrice = (double) session.getAttribute("price");
        ...
        try {
            ...
            parInfo.add(new Phrase(Chunk.NEWLINE));
            ///------Image-
            String absolutepath = getServletContext().getRealPath(logo);
            ...
            par.add(new Phrase("Depature:  " + depature, font));
            ...
            List<CustomerList> cusList = (List<CustomerList>) session.getAttribute("ListPasseger");
            ...
            for (int i = 0; i < cusList.size(); i++) {
                ...
                if ((i + 1) == (cusList.size())) {
                    ...
                    cell = new PdfPCell(new Paragraph(rooms + "", FontFactory.getFont("Arial", 12, Font.BOLD, BaseColor.BLACK)));
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    cell.setPadding(5);
                    table.addCell(cell);
                    ...
                    cell = new PdfPCell(new Paragraph(unitPrice + "$", FontFactory.getFont("Arial", 12, Font.BOLD, BaseColor.BLACK)));
                    cell.setHorizontalAlignment(Element.ALIGN_CENTER);
                    cell.setPadding(5);
                    table.addCell(cell);
